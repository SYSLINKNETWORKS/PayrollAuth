...
namespace TWP_API_Auth.Bussiness {
    public class BUserMenu : AbsBusiness {
        private readonly DataContext _context;
        private ErrorLog _ErrorLog = new ErrorLog ();
        String _UserId = "";
        public BUserMenu (DataContext context) {
            _context = context;
        }
        ...
        public override async Task<ApiResponse> GetDataAsync (ClaimsPrincipal _User) {
            ...
            try {
                var _Table = await _context.UserMenus.Include (x => x.userMenuModule).Include (x => x.userMenuSubCategory).Where (a => a.Action != Enums.Operations.D.ToString ()).ToListAsync ();
        ...
        public override async Task<ApiResponse> GetDataByIdAsync (Guid _Id, ClaimsPrincipal _User) {
            var ApiResponse = new ApiResponse ();
            try {
                var _Table = await _context.UserMenus.Include (x => x.userMenuModule).Include (x => x.userMenuSubCategory).Where (a => a.Id == _Id && a.Action != Enums.Operations.D.ToString ()).FirstOrDefaultAsync ();
        ...
        public override async Task<ApiResponse> AddAsync (object model, ClaimsPrincipal _User) {
            ...
            try {
                ...
                var _model = (UserMenu) model;
                var _Id = new Guid ();
                string error = "";
                bool aliasExists = _context.UserMenus.Any (rec => rec.Alias.Trim ().ToLower ().Equals (_model.Alias.Trim ().ToLower ()) && rec.ModuleId == _model.ModuleId && rec.SubCategoryId == _model.SubCategoryId && rec.Action != Enums.Operations.D.ToString ());
                bool nameExists = _context.UserMenus.Any (rec => rec.Name.Trim ().ToLower ().Equals (_model.Name.Trim ().ToLower ()) && rec.ModuleId == _model.ModuleId && rec.SubCategoryId == _model.SubCategoryId && rec.Action != Enums.Operations.D.ToString ());
                ...
                await _context.UserMenus.AddAsync (_model);
        ...
        public override async Task<ApiResponse> UpdateAsync (object model, ClaimsPrincipal _User) {
            ...
            try {
                ...
                string error = "";
                bool aliasExists = _context.UserMenus.Any (rec => rec.Alias.Trim ().ToLower ().Equals (_model.Alias.Trim ().ToLower ()) && rec.ModuleId == _model.ModuleId && rec.SubCategoryId == _model.SubCategoryId && rec.Action != Enums.Operations.D.ToString () && rec.Id != _model.Id);
                bool nameExists = _context.UserMenus.Any (rec => rec.Name.Trim ().ToLower ().Equals (_model.Name.Trim ().ToLower ()) && rec.ModuleId == _model.ModuleId && rec.SubCategoryId == _model.SubCategoryId && rec.Action != Enums.Operations.D.ToString () && rec.Id != _model.Id);
                ...
                var result = await _context.UserMenus.Where (a => a.Id == _model.Id && a.Action != Enums.Operations.D.ToString ()).FirstOrDefaultAsync ();
                if (result == null) {
                    ApiResponse.statusCode = StatusCodes.Status404NotFound.ToString ();
                    ApiResponse.message = "Record not found ";
                    return ApiResponse;
                ...
                result.ModuleId = _model.ModuleId;
                result.SubCategoryId = _model.SubCategoryId;
                ...
                // var _authclaim = await _context.AuthClaims.Where (a => a.Menu_Id == _model.Id && a.Action != Enums.Operations.D.ToString ()).FirstOrDefaultAsync ();
                // _authclaim.ClaimType = _model.Name;
                // await _context.SaveChangesAsync ();
        ...
        public override async Task<ApiResponse> DeleteAsync (Guid _Id, ClaimsPrincipal _User) {
            ...
            try {
                ...
                var _Table = _context.UserMenus.Where (a => a.Id == _Id && a.Action != Enums.Operations.D.ToString ()).FirstOrDefault ();
                if (_Table == null) {
                    ApiResponse.statusCode = StatusCodes.Status404NotFound.ToString ();
                    ApiResponse.message = "Record not found ";
                    return ApiResponse;
