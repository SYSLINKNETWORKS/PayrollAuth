...
{
    public class BUserMenuSubCategory : AbsBusiness
    {
        private readonly DataContext _context;
        private ErrorLog _ErrorLog = new ErrorLog();

        String _UserId = "";
        public BUserMenuSubCategory(DataContext context)
        {
            _context = context;
        }
        ...
        public override async Task<ApiResponse> GetDataAsync(ClaimsPrincipal _User)
        {
            ...
            try
            {
                var _Table = await _context.UserMenuSubCategories.Include(x => x.userMenuCategory).Where(a => a.Action != Enums.Operations.D.ToString()).ToListAsync();
        ...
        public override async Task<ApiResponse> GetDataByIdAsync(Guid _Id, ClaimsPrincipal _User)
        {
            ...
            try
            {
                var _Table = await _context.UserMenuSubCategories.Include(x => x.userMenuCategory).Where(a => a.Id == _Id && a.Action != Enums.Operations.D.ToString()).FirstOrDefaultAsync();
        ...
        public override async Task<ApiResponse> AddAsync(object model, ClaimsPrincipal _User)
        {
            ...
            try
            {
                ...
                var _model = (UserMenuSubCategory)model;

                string error = "";
                bool _NameExists = _context.UserMenuSubCategories.Any(rec => rec.Name.Trim().ToLower().Equals(_model.Name.Trim().ToLower()) && rec.CompanyId.Equals(_model.CompanyId) && rec.Action != Enums.Operations.D.ToString());
                ...
                await _context.UserMenuSubCategories.AddAsync(_model);
        ...
        public override async Task<ApiResponse> UpdateAsync(object model, ClaimsPrincipal _User)
        {
            ...
            try
            {
                ...
                string error = "";
                bool _NameExists = _context.UserMenuSubCategories.Any(rec => rec.Name.Trim().ToLower().Equals(_model.Name.Trim().ToLower()) && rec.CompanyId == _model.CompanyId && rec.Action != Enums.Operations.D.ToString());
                ...
                var result = _context.UserMenuSubCategories.Where(a => a.Id == _model.Id && a.Action != Enums.Operations.D.ToString()).FirstOrDefault();
                if (result == null)
                {
                    ApiResponse.statusCode = StatusCodes.Status404NotFound.ToString();
                    ApiResponse.message = "Record not found ";
                ...
                result.CategoryId = _model.CategoryId;
        ...
        public override async Task<ApiResponse> DeleteAsync(Guid _Id, ClaimsPrincipal _User)
        {
            ...
            try
            {
                ...
                _UserId = _User.Claims.FirstOrDefault(c => c.Type == Enums.Misc.UserId.ToString())?.Value.ToString();
                var _Table = _context.UserMenuSubCategories.Where(a => a.Id == _Id && a.Action != Enums.Operations.D.ToString()).FirstOrDefault();
                if (_Table == null)
                {
                    ApiResponse.statusCode = StatusCodes.Status404NotFound.ToString();
                    ApiResponse.message = "Record not found ";
